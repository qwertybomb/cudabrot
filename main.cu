...
__device__
bool good(thrust::complex<float_> c) {
	// test H1 and H2

	float c2 = norm(c);
	thrust::complex<float_> d = c + thrust::complex<float_>(1.0, 0.0);
	float h1 = 256.0 * c2 * c2 - 96.0 * c2 + 32.0 * c.real() - 3.0;
	float h2 = 16.0 * (norm(d)) - 1.0;
	if (h1 > 0.0 && h2 > 0.0)
		return true;
	return false;
};
...
__device__
void mSet(thrust::complex<float_> c, thrust::complex<float_>* Set,int* iterations, int maxIterations) {
	/*if point is the main cardioid it will not escape*/
	if (!good(c)) { *iterations = 0; return; }

	auto z = thrust::complex<float_>(0);
	
	*iterations = 0;
	while (norm(z) <= 6 && *iterations < maxIterations)
	{
		z = z * z + c;
...
__global__
void generateSamples(thrust::complex<float_>* Set,int* iterations,int maxIterations, thrust::complex<float_> minr, thrust::complex<float_> mini) {
	/*create a random number generator for both the real values and imaginary values*/
	...
	for (; index < samples; index += stride){
		...
		thrust::complex<float_> c(map<float_t>(curand_uniform(&realRand), 0, 1, minr.real(), minr.imag()), map<float_t>(curand_uniform(&imagRand), 0, 1, mini.real(), mini.imag()));
		/*generate points*/
		mSet(c, Set+index * (unsigned long long)maxIterations, iterations + index, maxIterations);
	}
}
...
__global__
void addToHeatmap(heatmap* buffer, thrust::complex<float_>* Set, int* iterations,int* maxIterations,heatmap* maxValues, thrust::complex<float_> minr, thrust::complex<float_> mini) {
	unsigned long long index = blockIdx.x * blockDim.x + threadIdx.x;
	unsigned long long stride = blockDim.x * gridDim.x;
	for (; index < samples* maxIterations[3]; index += stride) {
		int currentIteration = (index % maxIterations[3]);
		if (currentIteration >= iterations[index / maxIterations[3]]) { continue; }
		float_ real = Set[index].real(), imag = Set[index].imag();
		if (real < minr.real() || real > minr.imag() || imag < mini.real() || imag > mini.imag()) continue;//if point is out of bounds continue

		int row = map<float_>(real, minr.real(), minr.imag(), 0, (float_)width - 1);
		int col = map<float_>(imag, mini.real(), mini.imag(), 0, (float_)height - 1);
...
int main()
{
	...
	/*allocate memory for stroring the number of iterations a point takes to escape*/
	int* iterations;
	...
	for (int i = 0; i < sampleSamples; ++i) {
		...
		addToHeatmap << <512, 1024 >> > (buffer, Set, iterations, iter, maxValues, minr, mini);
		cudaDeviceSynchronize();
	}
	...
	for (int i = 0; i < width; ++i) {
		for (int j = 0; j < height; ++j) {
			...
			image.set_pixel(i, j, png::rgb_pixel(getColor(buffer[index], maxValues[0]),
